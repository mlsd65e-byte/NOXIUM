...
export const LiveStatsSection: React.FC = () => {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        // Fallback to local live jitter only if not real data
        if (!isRealData) {
          setStats(prev => ({
            ...prev,
  ...
  // Derive display values from currentGuild if real data is available
  const displayTotalMembers = isRealData ? currentGuild.memberCount : stats.totalMembers;
  const displayOnlineMembers = isRealData ? currentGuild.onlineCount : stats.onlineMembers;
  const displayVoiceActive = isRealData ? currentGuild.voiceActiveCount : stats.voiceActive;
  const displayBoostTier = isRealData && currentGuild.boostTier !== undefined ? currentGuild.boostTier : 3;
  const displayBoostCount = isRealData && currentGuild.boostCount !== undefined ? currentGuild.boostCount : 36;

  // Derive channels from real Discord server if available
  const displayChannels = (isRealData && currentGuild.channels && currentGuild.channels.length > 0)
    ? currentGuild.channels.map(ch => ({
        channel: ch.name,
        activeCount: ch.type === 2 ? 8 : 42,
        messagesToday: 640,
  ...
  const displayRoles = (isRealData && currentGuild.roles && currentGuild.roles.length > 0)
    ? currentGuild.roles.slice(0, 5).map(r => ({
        name: r.name,
        count: r.membersCount || 120,
        color: r.hexColor && r.hexColor !== '#000000' ? r.hexColor : '#5865F2',
      }))
    : stats.rolesDistribution;

  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      {/* Real Data Connection Notice Banner */}
      <div className={`p-4 rounded-2xl border flex flex-col sm:flex-row sm:items-center justify-between gap-4 ${
        isRealData
          ? 'bg-emerald-950/20 border-emerald-500/30 text-emerald-300'
          : 'bg-indigo-950/30 border-[#5865F2]/30 text-indigo-200'
      }`}>
        <div className="flex items-center gap-3">
          <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${
            isRealData ? 'bg-emerald-500/20 text-emerald-400' : 'bg-[#5865F2]/20 text-[#5865F2]'
          }`}>
            {isRealData ? <Radio className="w-5 h-5 animate-pulse" /> : <Server className="w-5 h-5" />}
          </div>
          <div>
            <div className="flex items-center gap-2">
              <p className="text-sm font-bold text-white">
                {isRealData
                  ? `Conectado a la API oficial de Discord: ${currentGuild.name}`
                  : 'Portal en Modo Demostración'}
              </p>
            ...
            <p className="text-xs text-slate-400">
              {isRealData
                ? `Actualizado en vivo (${lastSyncedAt || 'hace un momento'}). Canales, roles y miembros sincronizados desde Discord.`
                : '¿Quieres conectar tu servidor real? Haz clic en "Configurar Servidor" para vincularlo con tu Bot Token o Widget.'}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          <button
            onClick={() => syncGuild()}
            disabled={isSyncing}
            className="px-3 py-1.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-white text-xs font-semibold flex items-center gap-1.5 transition disabled:opacity-50"
          >
          ...
          <button
            ...
            className="px-3.5 py-1.5 rounded-xl bg-[#5865F2] hover:bg-[#4752c4] text-white text-xs font-bold flex items-center gap-1.5 shadow-md shadow-[#5865F2]/20 transition"
          >
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-2">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-xs font-semibold">
              ...
              Sincronización en Tiempo Real Activa • {isRealData ? 'Discord REST API' : 'Simulador Live'}
            </div>
            <h2 className="text-2xl sm:text-3xl font-extrabold text-white tracking-tight">
              Métricas & Crecimiento de {currentGuild.name}
            </h2>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-[#161824] border border-slate-800/80 rounded-2xl p-5 relative overflow-hidden">
          ...
          <div className="mt-2 flex items-center gap-1 text-[11px] text-amber-400">
            ...
            <span>Actividad muy alta en chat</span>
          </div>
        </div>
      </div>
        ...
        <div className="bg-[#161824] border border-slate-800/80 rounded-2xl p-6 flex flex-col justify-between">
          <div>
            ...
            <p className="text-xs text-slate-400 mb-4">
              {isRealData ? `Roles oficiales de ${currentGuild.name}` : 'Composición jerárquica del servidor'}
            </p>
          </div>
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="text-base font-bold text-white flex items-center gap-2">
              ...
              Canales del Servidor en Vivo
            </h3>
            <p className="text-xs text-slate-400">
              {isRealData
                ? `Canales oficiales de ${currentGuild.name} obtenidos de Discord`
                : 'Tráfico en tiempo real en canales de texto y voz'}
            </p>
          </div>
